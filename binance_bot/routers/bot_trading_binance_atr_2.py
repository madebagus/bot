...
run_bot_atr_bp = Blueprint('run_bot_atr', __name__, url_prefix='/api/bot/')
...
def calculate_dynamic_allocation(available_balance, leverage, assets):
    ...
    for asset, atr in volatility_data.items():
        if atr != 0:
            ...
            # Debugging output
            print(f"Asset: {asset}, ATR: {atr}, Weight: {weight}, Safe Trade Amount: {safe_trade_amount}")
        else:
...
coin_pairs = ['ETHUSDT', 'SOLUSDT', 'BTCUSDT','BNBUSDT','DOTUSDT','MAGICUSDT','LTCUSDT','AVAXUSDT','ADAUSDT','LINKUSDT','LUNAUSDT','SHIBUSDT','ARBUSDT','SUIUSDT','BELUSDT','INJUSDT']
...
@run_bot_atr_bp.route('/start_atr')
def start_bot():
